...
def test_repo_fields():
    ...
    assert field_1 == first_field 
    assert first_field == rec_fields[1] 
    assert field_2 == rec_fields[0] 

def test_repo_irrigation():
    repo = RepoIrrigation(SessionLocal())
    state_enum = repo.set_irrigation_state(irrigation_enum.START)

    assert state_enum == True 
    assert irrigation_enum.START == repo.get_current_irrigation_state()
